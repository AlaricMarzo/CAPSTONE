...
# -*- coding: utf-8 -*-
"""
ExtraTrees monthly demand forecast (Top-N) with:
 - Top-10 best-sellers (coverage-aware)
 - Fixed 80/20 time split
 - Walk-forward 1-step MASE
 - Past/Present/Future bands, connected forecast line (holdout→future)
 - Legend outside, compact metrics (no Params, no MSE)
 - Auto-select best per SKU by MASE (tie: WAPE, then RMSE)
 - Croston-SBA for intermittent series
 - Exports: top10_bestsellers.csv, per-SKU *_forecast.csv, *_comparison.csv, and extratrees_summary.csv

Run:
  python extra_trees_model.py --input "cleaned/ANC - 4 YEARS (1).csv" --topn 10 --min_cov 0.7 --tune 1
"""
