...
def plot_with_bands(title, y_idx, y_vals, train_idx, test_idx, pred_series, metrics_row, future_fc, out_png):
    ...
    # Compact metrics table (no Params)
...
def holdout_predict(mode:str, y: pd.Series):
    ...
    # Candidate set by mode
    if mode == "rf_only":
        cands = {"RandomForest": yhat}
    else:
        cands = {"RandomForest": yhat, "Seasonal-Naive": y_sn}
...
def future_forecast(y: pd.Series, mode:str, extra: Dict[str,object]) -> pd.Series:
    ...
    if mdl_full is None:
        ...
        mdl_full = fit_model_rf(feats.drop(columns=["y"]).values, ytr)

    hist=y.copy(); preds_rf=[]
    for _ in range(FORECAST_STEPS):
        ...
        hist.loc[nxt]=yhat; preds_rf.append(yhat)
    fc_ml=np.array(preds_rf)
...
def main():
    ...
    ap.add_argument("--mode", type=str, choices=["auto","rf_only"], default="auto")
    args=ap.parse_args()
    ...
    # Choose top best-sellers for modeling
    ...
    for sku in top:
        ...
        # Plot single chosen model
        png = out_dir / f"rf_{y_name}.png"
        plot_with_bands(
        ...
        # Walk-forward MASE (1-step, using RF engine)
        preds=[]; trues=[]
        for i in range(max(24, SEASON_M+1), len(y)):
            ...
            try:
                ...
                if feats.empty: yhat=hist.iloc[-1]
                    ...
                    y_tr_raw=feats["y"].values; cap=np.quantile(y_tr_raw,0.995); ytr=np.log1p(np.minimum(y_tr_raw,cap))
                    mdl=fit_model_rf(feats.drop(columns=["y"]).values, ytr)
                    nxt=(hist.index[-1].to_period("M")+1).to_timestamp()
                    tmp=hist.copy(); tmp.loc[nxt]=hist.iloc[-1]
                    x=add_ts_features(tmp).drop(columns=["y"]).iloc[[-1]].values
    ...
    if rows:
        pd.DataFrame(rows).to_csv(out_dir_root/"rf_summary.csv", index=False)
        print(f"✅ Saved → {out_dir_root/'rf_summary.csv'}")
