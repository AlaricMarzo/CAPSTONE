# backend/analytics/extratrees_model.py
# ExtraTrees forecasting per SKU with product description and smart chronological split.
...
def run_file(path: Path):
    ...
    for sku in top_skus:
        ...
        plt.plot(fc_dates, fc_vals, "g-", label="ExtraTrees Forecast (t+1..t+6)", linewidth=2)
        plt.title(f"ExtraTrees Forecast — {label}")
        plt.xlabel("Month"); plt.ylabel("Qty")
        plt.legend(); plt.grid(alpha=0.25)

        # ---- bottom metric text block (consistent with your other models) ----
        footer = f"ExtraTrees → MAE {mae:.2f}, MSE {mse:.2f}, RMSE {rmse:.2f}, MAPE {mp:.2f}%"
        plt.gcf().text(0.5, -0.06, footer, ha="center", va="top", fontsize=9, linespacing=1.4)

        plt.tight_layout()
        out_img = out_dir / f"et_{clean_name(sku)}.png"
        ...
        rows.append({
            ...
            "et_mse": mse,
            "et_mape": mp,
            **{f"et_t+{i}": float(round(v, 2)) for i, v in enumerate(fc_vals, 1)}
        })
    ...
    # save results
    if rows:
        pd.DataFrame(rows).to_csv(out_dir / "et_results.csv", index=False)
