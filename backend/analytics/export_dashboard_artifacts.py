...
# -*- coding: utf-8 -*-
"""
Exports dashboard-ready CSVs by sweeping model outputs (LSTM, ExtraTrees, GB, classical TS, etc.)
and recomputing history analytics. Produces dataset-level summary CSVs:

- <slug>_model_performance.csv
- <slug>_all_models_forecast.csv
- <slug>_full_timeline.csv
- <slug>_demand_insights.csv
- <slug>_inventory_recommendations.csv
- <slug>_seasonal_analysis.csv
- <slug>_volatility_analysis.csv
- <slug>_time_series_summary.csv        <-- NEW (from ts_* folders)

Assumptions:
- Your individual model scripts already wrote:
  • per-SKU forecast files:
      - ml_<model>/<dataset_slug>/<sku>_forecast.csv
      - ts_*/<dataset_slug>/<sku>_forecast.csv   (or ts_*/<sku>_forecast.csv)
  • per-model summary CSVs:
      - ml_<model>/<dataset_slug>/<model>_summary.csv
      - ts_*/<dataset_slug>/ts_summary.csv (or ts_*/ts_summary.csv)
      - ts_*/<something>_summary.csv, random_*_summary.csv (best-effort ingest)
  • per-SKU plot PNGs (optional)

- This script reads the original input dataset to rebuild monthly history (for insights).

Run example:
  python export_dashboard_artifacts.py --input "cleaned/ANC - 4 YEARS (1).csv" \
    --lead_days 10 --service_lvl 0.95 --annual_holding 0.20 --order_cost 150.0 \
    --topn 50 --min_cov 0.70

Outputs to: public/forecasts/<dataset_slug>/*.csv
"""
