...
# -*- coding: utf-8 -*-
"""
Randomized backtesting (Monte Carlo) + representative LAST-h chart — v2
Upgrades:
- Robust pre-processing (STL dampening + winsorize, auto-log1p)
- Coverage-aware Top-10 export
- Bands (Past/Present/Future) + connected Selected Forecast (holdout→future)
- Legend outside; compact metrics (Model, MAE, RMSE, MAPE%, MASE, WAPE%, MPE%)
- Per-SKU best = lowest MASE (tie-break WAPE, then RMSE)
- Croston-SBA path for intermittent demand
"""
...
mpl.rcParams.update({"figure.dpi":220,"axes.titlesize":16,"axes.labelsize":12,
                     "legend.fontsize":10,"xtick.labelsize":10,"ytick.labelsize":10})

HERE = Path(__file__).resolve().parent
DATA_DIR = HERE / "cleaned"
OUT_ROOT = HERE / "ts_sarima-ets-holts(random-test)_v2"
OUT_ROOT.mkdir(parents=True, exist_ok=True)

SEASONAL_PERIODS = 12
DEFAULT_H    = 6     # size of each random holdout
DEFAULT_REPS = 12    # number of random holdouts
DEFAULT_TOPN = 10
DEFAULT_STEPS= 6
...
# ---------- plotting ----------
def plot_bands_connected(desc, y, train, test, holdout_map, selected_line, future, metrics_rows, out_png, title):
...
def random_holdouts(y: pd.Series, reps: int, h: int):
    n=len(y)
    # if short, fall back to last-h only
    if n < 24 + h:
        yield y.iloc[:-h], y.iloc[-h:]; return
    max_start = n - h
    # avoid very early splits
    candidates = list(range(24, max_start))
    random.shuffle(candidates)
    for s in candidates[:reps]:
        yield y.iloc[:s], y.iloc[s:s+h]
...
def main():
    ...
    ap.add_argument("--recent", type=int, default=36, help="cap training for speed/stability")
    ...
    ap.add_argument("--no_calibrate", dest="do_cal", action="store_false"); ap.set_defaults(do_cal=True)
    args=ap.parse_args()
    ...
    for f in files:
        ...
        for desc in top_tbl["Description"]:
            ...
            agg: Dict[str, list] = {}
            def push(name, a, mse_, r, mp, ms, wa, me):
                agg.setdefault(name, []).append((a, mse_, r, mp, ms, wa, me))

            if is_intermittent(y):
                for tr, te in random_holdouts(y, args.reps, args.h):
                    p=croston_sba(tr, steps=len(te)); p.index=te.index
                    push("Croston-SBA",
                         mean_absolute_error(te,p), mean_squared_error(te,p), rmse(te,p),
                         mape_safe(te.values,p.values), mase(te.values,p.values,tr.values,SEASONAL_PERIODS),
                         wape(te.values,p.values), mpe(te.values,p.values))
                # SN reference (not used for future here, just for report)
                for tr, te in random_holdouts(y, max(1,args.reps//3), args.h):
                    sn = seasonal_naive_forecast(tr, SEASONAL_PERIODS, len(te)); sn.index=te.index
                    push("Seasonal-Naive",
                         mean_absolute_error(te,sn), mean_squared_error(te,sn), rmse(te,sn),
                         mape_safe(te.values,sn.values), mase(te.values,sn.values,tr.values,SEASONAL_PERIODS),
                         wape(te.values,sn.values), mpe(te.values,sn.values))
            else:
                for tr, te in random_holdouts(y, args.reps, args.h):
                    tr = tr.iloc[-args.recent:] if len(tr)>args.recent else tr
                    # ETS
                    try:
                        efit,_=fit_ets_grid(tr, SEASONAL_PERIODS, use_log)
                        if efit is not None:
                            p=ets_forecast(efit, len(te), tr.index[-1], use_log); p.index=te.index
                            push("ETS", mean_absolute_error(te,p), mean_squared_error(te,p), rmse(te,p),
                                 mape_safe(te.values,p.values), mase(te.values,p.values,tr.values,SEASONAL_PERIODS),
                                 wape(te.values,p.values), mpe(te.values,p.values))
                    except: pass
                    # SARIMA
                    try:
                        sfit,_=fit_sarima_grid(tr, SEASONAL_PERIODS, use_log)
                        if sfit is not None:
                            p=sarima_forecast(sfit, len(te), tr.index[-1], use_log); p.index=te.index
                            push("SARIMA", mean_absolute_error(te,p), mean_squared_error(te,p), rmse(te,p),
                                 mape_safe(te.values,p.values), mase(te.values,p.values,tr.values,SEASONAL_PERIODS),
                                 wape(te.values,p.values), mpe(te.values,p.values))
                    except: pass
                    # SARIMAX
                    try:
                        xfit,_=fit_sarimax_month_dummies(tr, SEASONAL_PERIODS, use_log)
                        if xfit is not None:
                            p=sarimax_forecast(xfit, tr.index[-1], len(te), use_log); p.index=te.index
                            push("SARIMAX", mean_absolute_error(te,p), mean_squared_error(te,p), rmse(te,p),
                                 mape_safe(te.values,p.values), mase(te.values,p.values,tr.values,SEASONAL_PERIODS),
                                 wape(te.values,p.values), mpe(te.values,p.values))
                    except: pass
                    # Holt-Winters
                    try:
                        seasonal=len(tr)>=24
                        hw=ExponentialSmoothing(np.log1p(tr) if use_log else tr,
                                                trend="add",
                                                seasonal=("add" if seasonal else None),
                                                seasonal_periods=(SEASONAL_PERIODS if seasonal else None),
                                                damped_trend=True,
                                                initialization_method="estimated").fit(optimized=True, remove_bias=True)
                        p=hw.forecast(len(te)); p=np.expm1(p) if use_log else p; p.index=te.index
                        p=clip_nonneg(p)
                        push("Holt-Winters", mean_absolute_error(te,p), mean_squared_error(te,p), rmse(te,p),
                             mape_safe(te.values,p.values), mase(te.values,p.values,tr.values,SEASONAL_PERIODS),
                             wape(te.values,p.values), mpe(te.values,p.values))
                    except: pass
                    # Seasonal-Naive baseline
                    sn = seasonal_naive_forecast(tr, SEASONAL_PERIODS, len(te)); sn.index=te.index
                    push("Seasonal-Naive", mean_absolute_error(te,sn), mean_squared_error(te,sn), rmse(te,sn),
                         mape_safe(te.values,sn.values), mase(te.values,sn.values,tr.values,SEASONAL_PERIODS),
                         wape(te.values,sn.values), mpe(te.values,sn.values))

            # Save aggregated means per model
            if agg:
                rows=[]
                for name, vals in agg.items():
                    A=np.array(vals, float); means=A.mean(axis=0)
                    rows.append([name, *means.tolist()])
                df=pd.DataFrame(rows, columns=["model","MAE","MSE","RMSE","MAPE","MASE","WAPE","MPE"])
                df = df.sort_values(["MASE","WAPE","RMSE"], ascending=[True,True,True])
                df.to_csv(out_dir/f"{sanitize(desc)}_random_{args.reps}x{args.h}_summary.csv", index=False)

            # ---- Representative LAST-h chart + forward forecast (bands + connected line) ----
            if len(y) <= args.h: continue
            train_full, test = y.iloc[:-args.h], y.iloc[-args.h:]
            train = train_full.iloc[-args.recent:] if len(train_full)>args.recent else train_full

            holdout_map: Dict[str,pd.Series] = {}
            metrics: List[Tuple] = []

            if is_intermittent(y):
                cro=croston_sba(train, steps=len(test)); cro.index=test.index
                holdout_map["Croston-SBA"]=cro
                metrics.append(("Croston-SBA",
                                mean_absolute_error(test,cro), rmse(test,cro),
                ...
                future = croston_sba(y, steps=args.steps)
                best_name = "Croston-SBA"
                selected = pd.concat([cro, future]).sort_index()
                if args.do_cal:
                    bc = calibrate(test.values, cro.values)
                    future *= bc
                    selected.iloc[-len(future):] = future.values
                future.to_csv(out_dir/f"{sanitize(desc)}_forecast.csv", header=["forecast"])

            else:
                # Fit on the representative split
                # ETS
                ...
                metrics.append(("Seasonal-Naive",
                                ...
                                mape_safe(test.values,sn.values), mase(test.values,sn.values,train.values,SEASONAL_PERIODS),
                                wape(test.values,sn.values), mpe(test.values,sn.values)))

                # Winner by MASE → WAPE → RMSE on this split
                ...
                # bias calibration from holdout
                if args.do_cal and best_name in holdout_map:
                    bc = calibrate(test.values, holdout_map[best_name].values)
                    future = future * bc

                # connected selected line
                selected = pd.concat([holdout_map[best_name], future]).sort_index()
                future.to_csv(out_dir/f"{sanitize(desc)}_forecast.csv", header=["forecast"])

            # plot
            # Build compact metrics rows from holdout
            metrics_rows = [(n, mean_absolute_error(test, p), rmse(test, p),
                             mape_safe(test.values,p.values), mase(test.values,p.values,train.values,SEASONAL_PERIODS),
                             wape(test.values,p.values), mpe(test.values,p.values))
                            for n,p in holdout_map.items()]
            png = out_dir / f"{sanitize(desc)}_representative_last_{args.h}.png"
            plot_bands_connected(desc, y, train, test, holdout_map, selected, future,
                                 metrics_rows, png,
                                 title=f"SARIMA/ETS/SARIMAX/Holt-Winters — last {args.h}m holdout")
            ...
            for row in metrics_rows:
                if row[0]==best_name:
                    chosen_mase = row[4]
                    break
            sku_rows.append({"description":desc,"chosen":best_name,"MASE_holdout":chosen_mase,"plot":png.name})

        if sku_rows:
            pd.DataFrame(sku_rows).to_csv(out_dir/"mc_summary.csv", index=False)
            print(f"✅ Saved → {out_dir/'mc_summary.csv'}")
