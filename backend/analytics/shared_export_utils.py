...
"""
import argparse, warnings, re, sys, math, textwrap
...
# ---------- Plotting (no clipping, outside legend, adaptive table) ----------
def _wrap_title(desc: str, width: int = 60) -> str:
    return textwrap.fill(str(desc), width=width)

def plot_pretty_holdout(
    desc, train, test, preds_map, metric_rows, out_png, title_prefix,
    figw=15.0, figh=9.0, right=0.78, left=0.08, top=0.90, bottom=0.12
):
    ...
    # Actuals
    ax.plot(train.index, train.values, label="Training Data", lw=2, color="tab:blue")
    ax.plot(test.index,  test.values,  label="Test Data", color="black", lw=2)

    # Holdout predictions
    palette = {"ETS":"tab:orange","SARIMA":"tab:green","SARIMAX":"tab:red","Croston-SBA":"tab:cyan"}
    for name, series in (preds_map or {}).items():
        ax.plot(series.index, series.values, lw=2, ls="--", color=palette.get(name, None), label=f"{name} Forecast")
    ...
    ax.set_title(f"{title_prefix} — {_wrap_title(desc)}")
    ax.set_xlabel("Month"); ax.set_ylabel("Qty")

    # Legend outside with reserved right margin
    ...
    if table_data:
        ...
        # Adaptive sizing if many rows
        base_font = 10 if len(table_data) <= 6 else 9
        base_scale = 1.10 if len(table_data) <= 6 else 0.98
        tbl.auto_set_font_size(False)
        tbl.set_fontsize(base_font)
        tbl.scale(1, base_scale)

    # Manual layout to avoid clipping (no bbox_inches="tight")
...
def main():
    ...
    # ---- new plotting/layout knobs ----
    p.add_argument("--figw", type=float, default=15.0)
    p.add_argument("--figh", type=float, default=9.0)
    p.add_argument("--right", type=float, default=0.78, help="axes right margin (reserve space for legend)")
    p.add_argument("--left", type=float, default=0.08)
    p.add_argument("--topmargin", type=float, default=0.90)
    p.add_argument("--bottom", type=float, default=0.12)
    ...
    for f in files:
        ...
        for desc in top_by_frequency(monthly, args.top):
            ...
            # save the chart (with layout knobs)
            png_path = OUT_ROOT / f"{sanitize(desc)}_80_20.png"
            plot_pretty_holdout(
                desc, train, test, preds_map, metric_rows, png_path,
                title_prefix="SARIMA/ETS/SARIMAX — 80% Train, 20% Test",
