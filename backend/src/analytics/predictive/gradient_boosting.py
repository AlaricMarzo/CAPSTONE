# backend/analytics/gradient_boosting.py
# Gradient Boosting forecasting per SKU (80/20 split) — same schema as Random Forest
...
def run_file(path: Path):
    ...
    for sku in top_skus:
        ...
        plt.plot(fc_dates, fc_vals, "g-", label="Gradient Boosting Forecast")
        plt.title(f"Gradient Boosting Forecast — {sku}")
        plt.legend(); plt.tight_layout()
        plt.savefig(out_dir / f"gb_{clean_name(sku)}.png", dpi=150)
        plt.close()
