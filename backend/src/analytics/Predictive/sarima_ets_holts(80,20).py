...
# -*- coding: utf-8 -*-
"""
80/20 holdout with calibrated forecasts
Models: ETS (grid), SARIMA (grid), SARIMAX (month dummies), Holt-Winters fixed, Croston (intermittent)
Includes:
- Monthly aggregation from raw ANC
- Winsorization (caps spikes)
- Intermittent-demand routing (Croston-SBA)
- Recent-window training (default 24 months)
- Optional log1p
- Bias calibration from holdout (median true/pred ratio, clipped)
- Chart with table: MAE, MSE, RMSE, MAPE, MASE, WAPE, MPE
- Best-by-MASE 6-month forecast CSV
Run:
  python sarima_ets_holts.py --file "ANC - 4 YEARS (1).csv" --top 8 --steps 6
"""
...
OUT_ROOT = HERE / "ts_sarima-ets-holts(80,20)"
...
def top_by_frequency(monthly, n):
    f = monthly.groupby("Description")["Qty"].apply(lambda s:(s>0).sum())
    return f.sort_values(ascending=False).head(n).index.tolist()
...
def plot_pretty(desc, train, test, preds_map, metrics, out_png):
    ...
    palette={"ETS":"tab:orange","SARIMA":"tab:green","SARIMAX":"tab:red","Holt-Winters":"tab:purple","Croston-SBA":"tab:cyan"}
    for n,s in preds_map.items(): ax.plot(s.index, s.values, label=f"{n} Forecast", lw=2, ls="--", color=palette.get(n,None))
    ...
    ax.set_title(f"SARIMA/ETS/SARIMAX/Holt-Winters — 80% Train, 20% Test — {desc}")
...
def main():
    ...
    ap.add_argument("--recent", type=int, default=24, help="use last N months for fitting")
    ...
    for f in files:
        ...
        for desc in top_by_frequency(monthly, args.top):
            ...
            # 80/20
            n=len(y); k=max(1,int(round(n*0.20)))
            ...
            if is_intermittent(y):
                ...
                # Holt-Winters fixed (damped add-add)
                try:
            ...
            plot_pretty(desc, train, test, preds_map, metrics, OUT_ROOT/f"{sanitize(desc)}_80_20.png")
