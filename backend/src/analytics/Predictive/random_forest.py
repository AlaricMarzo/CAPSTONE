...
def fit_model_rf(Xtr, ytr_log):
    ...
    model.fit(Xtr, ytr_log); return model, {"n_estimators":600,"min_samples_leaf":2,"max_features":"sqrt"}
...
def train_eval_holdout(y: pd.Series, h:int):
    ...
    mdl, params = fit_model_rf(feats_tr.drop(columns=["y"]).values, ytr_log)
    ...
    rows=[met("RandomForest", yhat, params), met("Seasonal-Naive", y_sn, f"m={SEASON_M}"), met("Blend", yb, f"alpha={best_a:.2f}, bias={bc:.2f}")]
    te_idx=feats_te.index
    pred_map={"RF holdout":pd.Series(yhat,index=te_idx),"SN holdout":pd.Series(y_sn,index=te_idx),"Blend (bias-corr)":pd.Series(yb,index=te_idx)}
    return train, test, pred_map, rows, best_a, bc, mdl, params

def fit_fn_rf_1step(hist: pd.Series):
    ...
    mdl,_=fit_model_rf(feats.drop(columns=["y"]).values, ytr)
...
def main():
    ...
    ap.add_argument("--input", type=str, help="Path to input CSV file (optional, uses database if not provided)")
    ...
    for sku in top:
        ...
        for i in range(max(24,SEASON_M+1), len(y)):
            hist=y.iloc[:i]
            try: preds.append(fit_fn_rf_1step(hist)); trues.append(float(y.iloc[i]))
            except: pass
        wf_mase = mase(np.array(trues), np.array(preds), y.values[:-len(trues)] if trues else y.values, SEASON_M) if trues else np.nan

        png=out_dir/f"rf_{clean_name(sku)}.png"
        plot_with_table(f"RandomForest — last-{h} holdout — {sku} — {desc}", y.index[:-h], y.index, y.values, y.index[-h:], pred_map, table_rows, png)
        ...
        if feats.empty:
            ...
            fc_rf=np.array(preds_rf); fc_sn=seasonal_naive(y.values,FORECAST_STEPS,SEASON_M)
            fc=alpha*fc_sn+(1-alpha)*fc_rf; fc*=bias
    ...
    if rows:
        ...
        print(f" Saved to {out_dir/'rf_summary.csv'}")
