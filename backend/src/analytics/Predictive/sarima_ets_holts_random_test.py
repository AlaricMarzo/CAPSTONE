...
OUT_ROOT = HERE / "ts_sarima-ets-holts(random-test)"
OUT_ROOT.mkdir(parents=True, exist_ok=True)

SEASONAL_PERIODS=12; DEFAULT_H=6; DEFAULT_REPS=6; DEFAULT_TOPN=5; DEFAULT_STEPS=6
...
def plot_pretty(desc, train, test, preds_map, metrics, out_png, title):
...
def main():
    ...
    for f in files:
        ...
        for desc in top_by_frequency(monthly, args.top):
            ...
            agg: Dict[str, list] = {}
            def push(name, a,m,r,mp,ms,wa,me):
                agg.setdefault(name, []).append((a,m,r,mp,ms,wa,me))

            if is_intermittent(y):
                for tr, te in random_holdouts(y, args.reps, args.h):
                    p=croston_sba(tr, steps=len(te))
                    push("Croston-SBA",
                ...
                for tr, te in random_holdouts(y, args.reps, args.h):
                    # recent-window for each split
                    tr = tr.iloc[-args.recent:] if len(tr)>args.recent else tr
                    try:
                        efit,_=fit_ets_grid(tr, SEASONAL_PERIODS, args.use_log)
                        if efit is not None:
                            p=ets_forecast(efit, len(te), tr.index[-1], args.use_log); p.index=te.index
                            push("ETS", mean_absolute_error(te,p), mean_squared_error(te,p), rmse(te,p),
                                 mape_safe(te.values,p.values), mase(te.values,p.values,tr.values,SEASONAL_PERIODS),
                                 wape(te.values,p.values), mpe(te.values,p.values))
                    except: pass
                    try:
                        sfit,_=fit_sarima_grid(tr, SEASONAL_PERIODS, args.use_log)
                        if sfit is not None:
                            p=sarima_forecast(sfit, len(te), tr.index[-1], args.use_log); p.index=te.index
                            push("SARIMA", mean_absolute_error(te,p), mean_squared_error(te,p), rmse(te,p),
                                 mape_safe(te.values,p.values), mase(te.values,p.values,tr.values,SEASONAL_PERIODS),
                                 wape(te.values,p.values), mpe(te.values,p.values))
                    except: pass
                    try:
                        xfit,_=fit_sarimax_month_dummies(tr, SEASONAL_PERIODS, args.use_log)
                        if xfit is not None:
                            p=sarimax_forecast(xfit, tr.index[-1], len(te), args.use_log); p.index=te.index
                    ...
                    try:
                        seasonal=len(tr)>=24
                        hw=ExponentialSmoothing(np.log1p(tr) if args.use_log else tr,
                        ...
                        p=hw.forecast(len(te)); p=np.expm1(p) if args.use_log else p; p.index=te.index
            ...
            if agg:
                ...
                df.to_csv(OUT_ROOT/f"{sanitize(desc)}_random_{args.reps}x{args.h}_summary.csv", index=False)

            # ---- Representative LAST-h chart + forward forecast ----
            ...
            if is_intermittent(y):
                ...
                if metrics:
                    ...
                    elif best=="SARIMAX" and xfit is not None: fc6=sarimax_forecast(xfit, y.index[-1], args.use_log and args.steps, args.use_log)
                    ...
                    else: fc6 = croston_sba(y, steps=args.steps)
                    if args.do_cal and best in preds_map:
                        fc6 = fc6 * calibrate(test.values, preds_map[best].values)
                    fc6.to_csv(OUT_ROOT/f"{sanitize(desc)}_forecast.csv", header=["forecast"])

            plot_pretty(desc, train, test, preds_map, metrics, OUT_ROOT/f"{sanitize(desc)}_representative_last_{args.h}.png",
