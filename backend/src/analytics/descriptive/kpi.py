...
# --- make axes occupy the full canvas ---
mpl.rcParams["figure.dpi"] = 120
mpl.rcParams["savefig.bbox"] = "tight"
mpl.rcParams["figure.constrained_layout.use"] = True  # key change

def _new_fig(figsize=(16, 10)):
    """Create a tall figure that uses the full drawing area (constrained layout)."""
...
def _finalize(fig: plt.Figure, path: Path):
    fig.savefig(path, dpi=240)  # no extra bbox adjustments needed with constrained layout
...
def _normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in ["date","receipt","item_code","description","qty","sales","cost","discount","profit","payment","cashier_id","category","tab"]:
...
def _set_zoom_limits(ax, ydata, lower_q=2, upper_q=98):
    y = pd.Series(ydata).dropna().astype(float)
    if len(y) == 0: return
    lo, hi = np.percentile(y, [lower_q, upper_q])
...
def _plot_monthly(monthly: pd.DataFrame, out_dir: Path):
    ...
    # SALES (tall)
    fig, ax = _new_fig(figsize=(18, 11))
    ax.plot(monthly_sorted["month"], monthly_sorted["total_sales"], marker="o")
    ...
    # QTY (tall)
    fig, ax = _new_fig(figsize=(18, 11))
    ax.plot(monthly_sorted["month"], monthly_sorted["total_qty"], marker="o")
...
def _plot_yearly(yearly: pd.DataFrame, out_dir: Path):
    ...
    _fmt_int(ax, y_numeric=True)
    _set_zoom_limits(ax, yearly["total_qty"])
...
def _plot_season_index_heatmap(season: pd.DataFrame, out_dir: Path):
    ...
    # SCALE HEIGHT BY #CATEGORIES — bigger per-row height so top/bottom are readable
    fig_height = max(12, len(pivot) * 0.70)
    fig = plt.figure(figsize=(18, fig_height), layout="constrained")
    ax = fig.add_subplot(111)

    im = ax.imshow(pivot.values, aspect="auto", norm=norm, cmap="viridis")
    ax.set_title("Season Index Heatmap (Category × Month)\n(1.0 = category's average month; >1 above, <1 below)")
    ...
    cbar.set_label("Season Index (centered at 1.0)")
...
def compute_kpis(df: pd.DataFrame, out_dir: str) -> Dict[str, Any]:
    ...
    monthly = df.groupby("month", as_index=False).agg(total_sales=("sales","sum"),
                                                      total_qty=("qty","sum"))
    yearly  = df.groupby("year", as_index=False).agg(total_sales=("sales","sum"),
                                                     total_qty=("qty","sum"))

    base_cols = ["item_code","description","category","tab"]
    by_prod = df.groupby(base_cols, as_index=False).agg(total_sales=("sales","sum"),
                                                       total_qty=("qty","sum"))
    ...
    # Splits
    cat_month = df.groupby(["month","category"], as_index=False).agg(total_sales=("sales","sum"),
                                                                     total_qty=("qty","sum"))
    tab_month = df.groupby(["month","tab"], as_index=False).agg(total_sales=("sales","sum"),
                                                                total_qty=("qty","sum"))
    ...
    active_items_monthly = (df.dropna(subset=["item_code"])[["month","item_code","description","category","tab"]]
                              .drop_duplicates()
    ...
    season = season[["category","month_num","month_sales","avg_sales","season_index"]].sort_values(["category","month_num"])
