...
interface HeaderNavProps {
  username?: string;
  onLogout: () => void; // <-- new prop to trigger real logout
}
...
export function HeaderNav({ username = "Admin User", onLogout }: HeaderNavProps) {
  ...
  // Confirmed logout
  const confirmLogout = () => {
    onLogout(); // switches view back to Login
    toast({
      title: "Logged out",
    ...
    <div className="flex items-center gap-4">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-56">
          ...
          {/* Logout with confirmation popout */}
          <AlertDialog open={confirmOpen} onOpenChange={setConfirmOpen}>
            <AlertDialogTrigger asChild>
              <DropdownMenuItem
                className="cursor-pointer text-destructive"
                onSelect={(e) => {
                  // prevent the menu from closing immediately, let AlertDialog handle it
                  e.preventDefault();
                  setConfirmOpen(true);
                }}
              >
